...
/**
 * I10000: ปัจจุบันอยู่ในอุตสาหกรรมตาม พ.ร.บ. ประกอบธุรกิจของคนต่างด้าว บัญชีที่ 1
 * - ใช้ตาราง Industry_company
 * - เงื่อนไข category_i = 1 (List 1)
 * - เรียง rec_update_when ใหม่สุดก่อน (null ไปท้าย) จากนั้น id ลดหลั่น
 * - รองรับ pagination (skip/take)
 */
export async function resolve_I10000(
  company_id: number,
  skip: number,
  take: number
  ...
  // นับจำนวนทั้งหมดที่อยู่ใน List 1
  const total = await prisma.industry_company.count({
    where: { company_id, category_i: 1 },
  });
  ...
  const rowsRaw = await prisma.industry_company.findMany({
    where: { company_id, category_i: 1 },
    orderBy: [
      // Prisma SQL Server ยังไม่รองรับ nullsLast โดยตรง จึงจัดด้วย 2 คีย์
