...
export async function resolve_S20000(company_id: number, skip: number, take: number): Promise<DetailResult> {
  const cols = [
  ...
  const [total, rows] = await prisma.$transaction([
    prisma.shareholder_s20000.count({ where: { company_id } }),
    prisma.shareholder_s20000.findMany({
      where: { company_id }, orderBy: [{ sh_upd_date: "desc" }], skip, take,
      select: { sh_upd_date: true, shareholder_name_th: true, shareholder_name_en: true, nation_th: true, nation_en: true, percent_share: true },
    }),
  ]);
